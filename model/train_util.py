...
class TrainLoop:
    ...
    def run_eval(self):
        self.model.eval()
        sample_fn = (
            self.val_diffusion.p_sample_loop if not self.val_ddim else self.val_diffusion.ddim_sample_loop
        )
        ...
        with th.no_grad():
            for batch in self.val_data:
                ...
                samples = sample_fn(
                    self.model, 
                    (gt.shape[0],1,dim,dim,dim),
                    model_kwargs={"cond": cond}, clip_denoised=True,
                )
                for i in range(samples.shape[0]):
                    sample_item = samples[i].squeeze().cpu().numpy()
                    gt_item = gt[i].squeeze().cpu().numpy()
                    cd, iou, l1 = self.evaluation.evaluate_tsdf(gt_item, sample_item, dim)
        ...
        logger.logkv("eval_step", self.step + self.resume_step)
        logger.dumpkvs()
        self.model.train()
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            micro = batch[i : i + self.microbatch].to(utils.dev())
            micro_cond = {
                "cond": cond[i : i + self.microbatch].to(utils.dev()),
            }

            t, weights = self.schedule_sampler.sample(micro.shape[0], utils.dev())
